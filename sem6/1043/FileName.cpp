...
TipCombustibil getTipCombustibilAsEnum(int tipCombustibil) {
...
string getTipCombustibilNiceDisplay(TipCombustibil tipCombustibil) {
	switch (tipCombustibil) {
	case Benzina:
		return "Benzina";
	case Motorina:
...
public:
	...
	Masina() :ID(0) {
		...
		this->anFabricatie = 0;
		this->nrKm = 0.0;
		this->tipCombustibil = Necunoscut;
	}

	Masina(int ID, string culoare, int anFabricatie, double nrKm, TipCombustibil tipCombustibil) : ID(ID) {
	...
	void setTipCombustibil(TipCombustibil tipCombustibil) {
		this->tipCombustibil = tipCombustibil;
	}
	...
	void afisareMasina() {
		cout << "ID: " << ID << " culoare: " << culoare << " anFabricatie: " << anFabricatie
			<< " nrKm: " << nrKm << " tipCombustibil: " << getTipCombustibilNiceDisplay(tipCombustibil) << endl;
	}
...
ostream& operator<<(ostream& afisConsola, const Masina& m) {
	afisConsola << "ID: " << m.ID << " culoare: " << m.culoare << " anFabricatie: "
		<< m.anFabricatie << " nrKm: " << m.nrKm << " tipCombustibil: " << getTipCombustibilNiceDisplay(m.tipCombustibil) << endl;
...
istream& operator>>(istream& citireConsola, Masina& m) {
	...
	int aux;
	cout << "tipCombustibil: "; citireConsola >> aux;

	m.tipCombustibil = getTipCombustibilAsEnum(aux);
...
public:
	...
	Tir(float tonajMaxim, int nrRemorci, int ID, string culoare, int anFabricatie, double nrKm, TipCombustibil tipCombustibil)
		:Masina(ID, culoare, anFabricatie, nrKm, tipCombustibil) {
		this->tonajMaxim = tonajMaxim;
		this->nrRemorci = nrRemorci;
	}
...
public:
	...
	Motocicleta(bool areCasca, string tipCadru, int ID, string culoare, int anFabricatie, double nrKm, TipCombustibil tipCombustibil)
		:Masina(ID, culoare, anFabricatie, nrKm, tipCombustibil) {
...
public:
	...
	Companie() {
		...
		this->pMasini = nullptr;

		this->pDenumire = new char[strlen("defaultName") + 1];
		strcpy(this->pDenumire, "defaultName");
	}

	Companie(int nrMasini, Masina* pMasini, const char* pDenumire) {
		if (nrMasini < 0) {
			throw "nrMasini trebuie sa fie >=0";
		}

		if (pMasini == nullptr) {
			exception ex("pMasini este nullptr");

			throw ex;
		}

		if (pDenumire == nullptr) {
			throw exception("pDenumire este nullptr");
		}
		...
		this->pDenumire = new char[strlen(pDenumire) + 1];
		strcpy(this->pDenumire, pDenumire);
	}
	...
	Companie(const Companie& c) {
		...
		this->pDenumire = new char[strlen(c.pDenumire) + 1];
		strcpy(this->pDenumire, c.pDenumire);
	}
	...
	Companie& operator=(const Companie& c) {
		if (this != &c) {
			...
			delete[] this->pDenumire;  //evitare memory leaks
	...
	void setPDenumire(char* pDenumire) {
		if (pDenumire == nullptr) {
			throw exception("pDenumire este nullptr");
		}
	...
	void afisareCompanie() {
		cout << "Companie cu denumirea " << pDenumire << ", " << nrMasini << " masini si caen " << caen << ":\n";
	...
	friend ostream& operator<<(ostream& afisConsola, const Companie& c);
	friend istream& operator>>(istream& citireConsola, Companie& c);
};

ostream& operator<<(ostream& afisConsola, const Companie& c) {
	afisConsola << "Companie cu denumirea " << c.pDenumire << ", " << c.nrMasini << " masini si caen " << c.caen << ":\n";
...
string Companie::caen = "111111";

istream& operator>>(istream& citireConsola, Companie& c) {
	cout << "Suprascriere Companie:\n";
	cout << "nr masini: "; citireConsola >> c.nrMasini;

	char aux[100];
	cout << "denumire: "; citireConsola >> aux;

	delete[] c.pDenumire; //evitare memory leaks
	c.pDenumire = new char[strlen(aux) + 1];
	strcpy(c.pDenumire, aux);

	cout << "Citire masini:\n";
	delete[] c.pMasini;//evitare memory leaks
	c.pMasini = new Masina[c.nrMasini];

	for (int i = 0; i < c.nrMasini; i++) {
		citireConsola >> c.pMasini[i];
	}
...
void main() {
	...
	Masina m2(20, "verde", 2010, 46600.6, Motorina);
	m2.afisareMasina();
	...
	try {
		Companie c3(11, nullptr, nullptr);
	}
	catch (const char* p) {
		cout << "Eroare: " << p << endl;
	}
	...
	Tir t11;
	Tir t12(10.4, 1, 1399, "alb", 2019, 700000.559, Motorina);
	...
	Motocicleta moto1;
	Motocicleta moto2(true, "cruiser", 1444, "rosu", 2025, 6700.44, Motorina);
